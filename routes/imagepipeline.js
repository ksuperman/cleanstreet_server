            ...
            for(var i = 0;i < imagesParsed.length; i++){
                ...
                if(imagesParsed[i].detectionOptimizedResults) {
                    ...
                    for( var j = 0; j < imagesParsed[i].detectionOptimizedResults.length; j++) {
                        ...
                        tempBoundingBoxArea = (tempResultObject.bb.w * tempResultObject.bb.h);
                        imagesParsed[i].detectionOptimizedResults[j].bbArea = tempBoundingBoxArea;
...
router.get('/image/:imageid', function (req, res, next) {
    ...
    if (req.params && req.params.imageid >= 0) {
        image_pipeline_model.findOne({id: req.params.imageid}, {exifObj:0}).lean().exec(function (err, imageObject) {
            if (err || !imageObject) {
                console.log(err);
