...
mongoexport --host $1 --db clean_street --collection images --jsonArray --query '{"status": "N", "image_type":"training"}' --fields coco_url,date_captured,file_name,flickr_url,height,width,id,license | sed '/"_id":/s/"_id":[^,]*,//g' >> ./temp/images.json
mongoexport --host $1 --db clean_street --collection images --jsonArray --query '{"status": "N", "image_type":"validate"}' --fields coco_url,date_captured,file_name,flickr_url,height,width,id,license | sed '/"_id":/s/"_id":[^,]*,//g' >> ./temp/validationImages.json
